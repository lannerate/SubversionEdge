    ...
    for opt, arg in opts:
        ...
        if opt in ("-d", "--basedir"):
            ...
            if not os.path.exists(basedir):
                print "The system cannot find the basedir specified: '%s'"%basedir
                sys.exit(1)
...
def start_parsing(include_dirs):
    for include_dir in include_dirs:
        if include_dir in os.listdir(basedir):
            ...
            print 'The include directory("%s") is not present in the basedir("%s").'%(include_dir, basedir)

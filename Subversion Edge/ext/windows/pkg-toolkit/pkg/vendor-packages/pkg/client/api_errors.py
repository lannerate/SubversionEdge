...
class UnknownUser(ActionExecutionError):
        """Used to indicate that an action has an owner tag that is unknown."""
        def __init__(self, action, user):
                ActionExecutionError.__init__(self, action, None,
                    _("unknown user \"%(user)s\" required to install %(path)s") % {
                    "user" : user, "path": action.attrs["path"]})
                self.user = user

class UnknownGroup(ActionExecutionError):
        """Used to indicate that an action has an group tag that is unknown."""
        def __init__(self, action, group):
                ActionExecutionError.__init__(self, action, None,
                    _("unknown group \"%(group)s\" required to install %(path)s") % {
                    "group" : group, "path": action.attrs["path"]})
                self.group = group

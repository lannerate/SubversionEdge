...
rem  this script.
set _WRAPPER_CONF_DEFAULT=../data/conf/csvn-wrapper.conf
...
rem
rem artf62798 moving the wrapper configuration in case the directory "data" does not exist. 
rem
if not exist "%_REALPATH%..\data" goto createDirs

:createDirs
mkdir "%_REALPATH%..\data" > nul 2>&1
mkdir "%_REALPATH%..\data\conf" > nul 2>&1
mkdir "%_REALPATH%..\data\logs" > nul 2>&1
mkdir "%_REALPATH%..\data\run" > nul 2>&1
mkdir "%_REALPATH%..\data\repositories" > nul 2>&1

if exist "%_REALPATH%..temp-data\conf\csvn-wrapper.conf" goto: moveconf

:moveconf
move "%_REALPATH%..\temp-data\conf\csvn-wrapper.conf" "%_REALPATH%..\data\conf\" > nul 2>&1
